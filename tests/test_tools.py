...
async def test_request_document_receita_nuit_flash_orienta_retirada():
    """Nuit Flash entra na lista de receita física: responde orientando retirada
    presencial e notifica a clínica com o aviso de receita física."""
    from app.graph.tools import request_document
    client, _, _ = _make_supabase_client()
    with patch("app.graph.tools.get_supabase", new_callable=AsyncMock, return_value=client), \
         patch("app.graph.tools.log_event", new_callable=AsyncMock), \

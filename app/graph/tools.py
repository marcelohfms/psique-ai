...
async def request_external_contact(
    ...
    try:
        ...
        # Sem isto, qualquer falha (ex.: tabela `requests` ausente) sobe como
        # "Erro interno" e a Eva fica muda — o paciente não recebe resposta.
        logger.exception("request_external_contact failed phone=%s", phone)
        try:
            await log_event("external_contact_request_failed", phone_clean, {
                "third_party_role": third_party_role,
                "third_party_name": third_party_name,
                "reason": reason,
            })
            ...
            pass
        # Best-effort: sinaliza a clínica pra não perder o pedido silenciosamente.
        try:
            fail_msg = (
                f"⚠️ Falha ao registrar automaticamente um pedido de contato externo — verifique manualmente.\n"
                f"Terceiro: {third_party_name} ({third_party_role})\n"
                f"Médico(a): {doctor_label}"
            )
            if reason:
                fail_msg += f"\nMotivo: {reason}"
            await _notify_clinic(fail_msg, phone=phone_clean, subject="Falha — pedido de contato externo")
        except Exception:
            pass
        return (
            "Recebi seu pedido e já sinalizei para a nossa equipe. 😊 "
            "Em breve retornamos por aqui com o encaminhamento."
        )
...
async def nudge_external_contact(
    ...
    try:
        ...
        # Sem isto, qualquer falha (ex.: tabela `requests` ausente) sobe como
        # "Erro interno" e a Eva fica muda — o paciente não recebe resposta.
        logger.exception("nudge_external_contact failed phone=%s", phone)
        try:
            await log_event("external_contact_nudge_failed", phone_clean, {
                "patient_message": patient_message,
            })
        except Exception:
            pass
        return (
            "Recebi sua mensagem e já sinalizei sua cobrança para a nossa equipe. 😊 "
            "Assim que houver retorno sobre o contato, avisamos você por aqui."
        )

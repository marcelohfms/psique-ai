...
Queries tomorrow's scheduled appointments from the DB, groups by doctor,
and sends one email per doctor with the full list.
"""
import asyncio
import os
import smtplib
...
async def main():
    ...
    # Use Recife-timezone-aware boundaries so PostgREST interprets them correctly
    time_min = datetime(tomorrow.year, tomorrow.month, tomorrow.day, 0, 0, 0, tzinfo=TZ)
    time_max = datetime(tomorrow.year, tomorrow.month, tomorrow.day + 1, 0, 0, 0, tzinfo=TZ)
    ...
    print(f"Tomorrow's appointments ({date_str}): {len(appointments)}")
    ...
    for doctor_id, appts in by_doctor.items():
        ...
        try:
            _send_email(doctor_email, subject, body)

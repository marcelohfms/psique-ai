...
async def main():
    ...
    for phone in _phone_variants(base):
        ...
        for u in users:
            ...
            appts = await client.from_("appointments").select("*").eq("patient_id", u.get("id")).order("start_time", desc=True).limit(10).execute()
            if appts.data:
                print("  Consultas:")
                for a in appts.data:
    ...
    # busca direta em contacts também, caso get_users_by_phone não capture tudo

"""
Força a leitura do comprovante de pagamento (taxa de reserva) enviado pela mãe do
paciente Fernando Daniel (5581994358739). O comprovante nunca chegou a ser processado
pela Eva (nenhum rastro no Supabase após a mensagem de confirmação da consulta às
31/07 13:05:47), possivelmente porque a imagem foi classificada como irrelevante
(IGNORAR) ou o download falhou silenciosamente.
...
Uso: uv run python scripts/_force_read_comprovante_5581994358739.py
"""
import asyncio
import datetime
import httpx
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=90) as client:
        ...
        convs = await cw._get_contact_conversations(client, contact["id"])
        conv = cw._pick_conversation(convs)
        if not conv:

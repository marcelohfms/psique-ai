...
PHONES = ["558189002986", "5581989002986", "8189002986"]
...
async def main():
    ...
    print("\n=== LAST MESSAGES ===")
    for phone in PHONES:
        msgs = await client.table("messages").select("*").eq("phone", phone).order("created_at", desc=True).limit(30).execute()
        if msgs.data:
            print(f"-- on {phone} --")
            for m in reversed(msgs.data):
                print(f" [{m.get('created_at')}] {m.get('role')}: {str(m.get('content'))[:220]}")
